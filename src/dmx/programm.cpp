...
void Programm::addDeviceProgramm(const QJsonObject &o) {
    programms.push_back(std::make_unique<DeviceProgramm>(ModelManager::get().getDeviceById(o["device"]), ModelManager::get().getProgramPrototypeById(o["programmPrototype"]), o["offset"].toDouble()));
...
bool Programm::addDeviceProgramm(Device *device, ProgrammPrototype *programmPrototype, double offset) {
    if (!device || !programmPrototype) return false;
