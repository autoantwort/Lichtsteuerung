...
ProgrammPrototype::ProgrammPrototype(const QJsonObject &o) : NamedObject(o), IDBase<ProgrammPrototype>(o), devicePrototype(ModelManager::get().getDevicePrototypeById(o["devicePrototype"])) {
...
ChannelProgramm *ProgrammPrototype::getChannelProgramm(const Channel *channel) {
    for (auto p = programm.begin(); p != programm.end(); ++p) {
        if ((**p).channel == channel) return p->get();
    }
    return nullptr;
}
ChannelProgramm *ProgrammPrototype::getChannelProgramm(int index) {
    for (auto p = programm.begin(); p != programm.end(); ++p) {
        if ((**p).channel->getIndex() == index) return p->get();
    }
    return nullptr;
}
