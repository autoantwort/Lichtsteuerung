...
public:
    ...
    Q_SLOT void setPosition(const QPoint &p) {
    ...
    Q_SLOT void setStartDMXChannel(unsigned int newStart) {
        if (newStart == startDMXChannel) return;
        startDMXChannel = newStart;
        emit startDMXChannelChanged(startDMXChannel);
    }
