...
Scanner::Scanner(QVector3D scannerPosition, float scannerRotationInRoomInDegree, float scannerTiltInDegree):scannerPosition(scannerPosition),scannerRotationInRoom(scannerRotationInRoomInDegree),scannerTilt(scannerTiltInDegree)
{
...
std::pair<float, float> Scanner::computeAngleForStepMotors(QVector3D positionInRoom){
    auto vecFromScanner = transformationMatrix * positionInRoom;
