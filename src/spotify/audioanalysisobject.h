...
public:
    /**
     * @brief bars The time intervals of the bars throughout the track. A bar (or measure) is a segment of time defined as a given number of beats. Bar offsets also indicate downbeats, the first beat of the measure.
     */
    ...
    /**
     * @brief sections Sections are defined by large variations in rhythm or timbre, e.g. chorus, verse, bridge, guitar solo, etc. Each section contains its own descriptions of tempo, key, mode, time_signature, and loudness.
     */
    ...
    /**
     * @brief tatums A tatum represents the lowest regular pulse train that a listener intuitively infers from the timing of perceived musical events (segments). For more information about tatums, see Rhythm (below).
     */
...
int AudioAnalysisObject::getIndexForTime(const std::vector<T> &vec, int ms)const{
    struct comperator{
        bool operator()(const T &l, const int r){
            return l.start*1000<r;
        }
        bool operator()(const int l, const T &r){
            return l<r.start*1000;
        }
    };
