...
public:
    ...
    type x, y;
    bool isNear(type x, type y, type maxDisstance) const { return (this->x - x) * (this->x - x) + (this->y - y) * (this->y - y) < maxDisstance * maxDisstance; }
...
public:
    ...
    inline bool haveIndex(Indextype i) const { return first == i || second == i || third == i; }
...
public:
    ...
    inline bool haveIndex(Indextype i) const { return left == i || mid == i || right == i; }
    void decrementAbove(Indextype i) {
        if (left > i) --left;
        if (mid > i) --mid;
        if (right > i) --right;
...
public:
     ...
     */
    void addRectangle(Point::number_type x1, Point::number_type y1, Point::number_type x2, Point::number_type y2, Point::number_type x3, Point::number_type y3, Point::number_type x4,
                      Point::number_type y4) {
