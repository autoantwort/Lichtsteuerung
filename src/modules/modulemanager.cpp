...
ModuleManager::ModuleManager() {
    ...
    consumer.emplace("DMXConsumer", "With the DMXConsumer class you can write to the DMX Channels", -1, []() { return new DMXConsumer; });
    // add virtual led consumer to test
    consumer.emplace("Virtual LED Consumer", "With this led consumer you can view the led stripes in the light control application", -1, []() { return new LedConsumer; });
}
...
void ModuleManager::loadModule(QString name, std::function<std::string(const std::string &)> replaceOldModulesInProgramBlocks) {
    ...
    if (!QLibrary::isLibrary(name)) return;
