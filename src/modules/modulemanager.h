...
namespace detail {
    ...
    public:
        ...
        Property::Type getType() const { return type; }
    signals:
        void nameChanged();
...
public:
    ...
    // Module() = default;
    const PropertiesVector &getProperties() const { return properties; }

    bool isSpotifyResponder() const { return spotifyResponder; }

    PropertiesVector *getPropertiesP() { return &properties; }
    ...
    QString getCompiledName() const { return compiledName; }
    void setDescription(const QString _description) {
        if (_description != description) {
            description = _description;
            emit descriptionChanged();
    ...
    ValueType getInputType() const { return inputType; }
    ...
    ValueType getOutputType() const { return outputType; }
...
namespace detail {
    ...
    public:
        ...
        bool operator<(const Entry<T> &other) const { return this->name_ < other.name_; }
    };
} // namespace detail
...
public:
    ...
    ~ModuleManager() {
        for (auto m : modules)
            delete m;
        QObject::disconnect(captureStatusChangedConnection);
    }
    ...
    static ModuleManager *singletone() {
...
protected:
    ...
    static std::shared_ptr<T> createByName(const std::set<detail::Entry<T>> &c, const std::string &name) {
        for (auto i = c.cbegin(); i != c.cend(); ++i) {
            if (i->name() == name) return std::shared_ptr<T>(i->create());
        }
        return nullptr;
    }
};
...
void ModuleManager::loadType(QLibrary &lib, ModuleContainer<detail::Entry<Type>> &c, String Typename, int libraryIdentifier, Callback callback) {
    ...
    if (!getNumberOfProgramms || !getNameOfProgramm || !getDescriptionOfProgramm || !createProgramm) {
        std::cout << "Functions are missing : " << getNumberOfName << " : " << getNumberOfProgramms << " , " << getNameOfName << " : " << getNameOfProgramm << " , " << getDescriptionOfName << " : "
                  << getDescriptionOfProgramm << " , " << createName << " : " << createProgramm;
        return;
    }
