...
namespace detail {
    ...
    public:
        bool operator()(const PointerType *l, const PointerType *r) const { return l < r; }
        bool operator()(const std::shared_ptr<PointerType> &l, const std::shared_ptr<PointerType> &r) const { return l.get() < r.get(); }
        bool operator()(const PointerType *l, const std::shared_ptr<PointerType> &r) const { return l < r.get(); }
        bool operator()(const std::shared_ptr<PointerType> &l, const PointerType *r) const { return l.get() < r; }
    };
} // namespace detail
...
std::vector<std::shared_ptr<Program>> ProgramBlock::getUsedProgramsByName(const std::string &name) const {
    ...
    for (const auto &p : programs) {
        if (std::strcmp(p->getName(), name.c_str()) == 0) vec.push_back(p);
    }
    return vec;
}
std::vector<std::shared_ptr<Consumer>> ProgramBlock::getUsedConsumersByName(const std::string &name) const {
    ...
    for (const auto &p : consumer) {
        if (std::strcmp(dynamic_cast<Named *>(p.source.get())->getName(), name.c_str()) == 0) vec.push_back(std::dynamic_pointer_cast<Consumer>(p.source));
    }
    return vec;
}
std::vector<std::shared_ptr<Filter>> ProgramBlock::getUsedFiltersByName(const std::string &name) const {
    ...
    for (const auto &p : filter) {
        if (std::strcmp(dynamic_cast<Named *>(p.second.source.get())->getName(), name.c_str()) == 0) vec.push_back(std::dynamic_pointer_cast<Filter>(p.second.source));
    }
    return vec;
}
...
void ProgramBlock::replaceFilter(std::shared_ptr<Filter> original, std::shared_ptr<Filter> newFilter) {
    ...
    if (foundOne) emit propertyBaseChanged(original.get(), newFilter.get());
}

void ProgramBlock::replaceConsumer(std::shared_ptr<Consumer> original, std::shared_ptr<Consumer> newConsumer) {
    if (std::find_if(std::cbegin(consumer), std::cend(consumer), [&](const auto &c) { return c.source == original; }) == std::end(consumer)) return;
