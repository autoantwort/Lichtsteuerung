...
public:
    ...
    LedConsumer();
    ~LedConsumer() override { allLedConsumer.removeAll(this); }
    QString getNameFromUser() const { return QString::fromStdString(name.getString()); }
    void setInputLength(unsigned int size) override { resize(size); }
    unsigned int getInputLength() const override { return static_cast<unsigned int>(size()); }
    const char *getName() const override { return "Virtual LED Consumer"; }
